...
export const PredictionWorkspace = () => {
  ...
  const numeric_columns = availableColumns.filter(col => col.type === "numeric");

  const [datasetName, setDatasetName] = useState(
    localStorage.getItem("uploaded_file_name") ||
    "No dataset uploaded"
  );

  useEffect(() => {
    const data = localStorage.getItem("analytics_results");
  ...
  if (
    parsed.numeric_columns &&
    parsed.numeric_columns.length > 0
  ) {
    setTarget(
            ...
            <div className="flex gap-2 pt-1">
              {[7, 14, 30, 60].map((n) => (
                <button
                  ...
                  className={`text-[11px] font-mono px-2.5 py-1 rounded-md border transition-colors ${Number(horizon) === n
