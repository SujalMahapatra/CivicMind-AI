    ...
    const runAnalysis = async () => {
        ...
        try {
            ...
            setDataset(prev => ({
                ...
                cols: response.data.total_columns
            }));
                        ...
                        {dataset && (
                            <div className="mt-5 rounded-xl border border-white/10 bg-white/[0.02] p-4 animate-in-up">
                                ...
                                <div className="mt-3 grid grid-cols-3 gap-3">
                                    ...
                                    <StatChip
                                        label="Cols"
                                        value={results?.total_columns?.toLocaleString?.() || "-"}
                                    />
                        ...
                        {results && (
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                                    {[
                                        { label: "Rows", value: results.rows },
                                        { label: "Columns", value: results.columns },
                                        { label: "Missing Values", value: results.missing_values },
                                        { label: "Duplicate Rows", value: results.duplicate_rows },
                                        { label: "Numeric Columns", value: results.numeric_columns },
                                    ].map((m) => (
                                        <StatChip
                                            key={m.label}
                                            label={m.label}
